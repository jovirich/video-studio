...
class GenerationJob:
    ...
    def to_operator_brief(self) -> str:
        ...
        add("")
        add("**Copy ONLY what is inside the box.** Nothing else on this page is part")
        add("of the prompt — the candidate count, the filenames, and the checklist are")
        add("instructions to you, not to the model. Pasting them in is how a request")
        add("for four images becomes a request for one image containing four faces.")
        add("")
        add("```text")
        ...
        if self.candidates > 1:
            add(f"Run the prompt **{self.candidates} separate times**.")
            add("")
            add(f"**{self.candidates} runs → {self.candidates} files → one subject in")
            add("each.** Not a grid, contact sheet, collage, montage, or multi-panel")
            add("image. A composite is a single file with a single hash, so it cannot")
            add("be ingested as separate candidates and cannot be approved or rejected")
            add("individually — which is the whole point of generating more than one.")
            add("")
            add("Do not ask the surface for several images at once if it offers that.")
            add("Run it again, from the top, each time.")
            add("")
