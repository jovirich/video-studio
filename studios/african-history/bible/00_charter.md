---
doc: bible/00
title: Charter
status: template
version: 0.1.0
owners: [showrunner]
---

# 00 — Charter

> **Fill state.** The structural commitments below are studio policy and stand as
> written. Fields marked `TBD` are deliberate: they are decisions for the
> Showrunner, not defaults to inherit.

## 1. Mission

`TBD — one sentence, written by the Showrunner, that survives being read aloud.`

Drafting constraints for that sentence:
- It must be falsifiable as a description of the work. "Telling untold stories" is
  not falsifiable and therefore not a mission.
- It must not promise comprehensiveness. A series cannot cover a continent.
- It must name the audience relationship, not just the subject.

## 2. Scope

**African History Studio** produces long-form documentary about the histories of the
African continent and its diasporas, organised into **production lines** by
geography.

| | |
|---|---|
| Line 01 | **Nigeria** — `productions/ng-nigeria/` |
| Future lines | Opened by decision of the Showrunner. Candidate lines are held in [../productions/README.md](../productions/README.md) § Candidate lines. |

A production line is opened when three conditions hold, not before:
1. A named research lead with domain competence has agreed to own it.
2. At least one advisory contact with standing in that region has agreed to review.
3. The archive landscape has been surveyed and recorded in the line's
   `sources/archive_landscape.md`.

Opening a line without (2) is the single most reliable way to produce work that
insults the people it claims to honour.

## 3. Audience

`TBD — Showrunner.` Define at least: primary audience, assumed prior knowledge,
and the reading age of the narration. These three drive nearly every downstream
decision in [03_narrative_doctrine.md](03_narrative_doctrine.md).

Whatever is chosen, one constraint is fixed: **the work must be legible and
non-condescending to an audience from the region it depicts.** A documentary about
Nigeria that only makes sense to people who have never met a Nigerian has failed
regardless of its viewing figures.

## 4. What this studio is

- A **history** studio. The subject is the past and the evidence for it.
- A **documentary** studio. Claims are answerable to sources.
- An **AI-assisted** studio. Generative tools do the work that archival material
  cannot: reconstruction, visualisation, scale. They do not do the work of
  evidence.

## 5. What this studio is not

- **Not a heritage-marketing operation.** Celebration is a legitimate tone; it is
  not a licence to omit. Where a subject is uncomfortable, the discomfort is part
  of the record.
- **Not a rebuttal service.** The work does not exist to answer other people's
  distortions. Correcting the record is a by-product of getting it right, not the
  organising purpose.
- **Not a synthetic-archive operation.** We generate imagery. We never present
  generated imagery as recovered evidence. See
  [06_ai_disclosure_and_ethics.md](06_ai_disclosure_and_ethics.md).
- **Not a single-narrative studio.** Where historians disagree, the disagreement is
  the content, not an obstacle to it.

## 6. Editorial independence

The Showrunner holds final editorial authority. Funders, platform partners,
advisors, and institutional lenders may advise; none may compel a change to a
factual claim or its framing.

Where an archive grants access on condition of editorial input, that condition is
recorded in [../sources/permissions/clearance_log.md](../sources/permissions/clearance_log.md)
and disclosed on screen. If the condition cannot be disclosed, the material is not
used.

The Cultural Advisor is the one exception to unilateral Showrunner authority: an
advisory hold under [07_cultural_sensitivity.md](07_cultural_sensitivity.md) §4
cannot be overridden by the Showrunner alone. It requires a documented ruling.

## 7. Standing commitments

These are binding and are enforced by review gates:

1. **Every factual claim is traceable to a source record.**
2. **Every generated asset is labelled and logged.**
3. **Errors are corrected publicly and permanently**, per
   [01_editorial_standards.md](01_editorial_standards.md) §4.
4. **Regional scholars and knowledge holders are credited by name and paid.**
   Consultation is labour, not a favour.
5. **Nothing sacred is depicted on the studio's own judgement.**

## 8. Success conditions

`TBD — Showrunner.` Define what would make this project worth having made,
separately from audience metrics. Suggested axes to choose from, not to adopt
wholesale: scholarly citation of the series, adoption in classrooms in the region
depicted, correction rate trending to zero, the advisory board agreeing to return
for a second season.

## 9. Review

This charter is reviewed at the close of each season and on the opening of each new
production line. Changes go through the amendment process in
[README.md](README.md) § How to change the Bible.
